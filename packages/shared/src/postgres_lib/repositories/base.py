...
"""
Generic type variable representing any SQLAlchemy ORM model inheriting from IdModel.

Used to parameterize repository interfaces while preserving static
...
class BaseRepository(ABC):
    ...
    Attributes
    ----------
    model_class : ClassVar[type[IdModelType]]
    ...
    argument. Transaction lifecycle — commit, rollback, and close — is managed
    externally by the caller, never inside the repository. Creation and update
    payloads are plain dictionaries, typically derived from validated Pydantic
    schemas. Update dictionaries must contain only mutable fields; primary keys
